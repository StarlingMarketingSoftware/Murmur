...
import { PostContactData } from '@/app/api/contacts/route';
import { appendQueryParamsToUrl } from '@/utils';
import { _fetch } from '@/utils';
import { urls } from '@/constants/urls';
import { PatchContactVerificationRequestData } from '@/app/api/contact-verification-requests/route';
	...
	return useMutation({
		mutationFn: async (data: PatchContactVerificationRequestData) => {
			...
			if (!response.ok) {
				const errorData = await response.json();
				throw new Error(
					errorData.error || 'Failed to update contact verification request'
				);
			}
...
export const useVerifyContacts = (options: CustomMutationOptions = {}) => {
	const {
		suppressToasts = false,
