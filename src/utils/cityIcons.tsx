...
import { VirginiaIcon, VIRGINIA_BACKGROUND } from '@/components/atoms/_svg/VirginiaIcon';
import {
	WashingtonIcon,
...
export const getCityIconProps = (city: string, state: string) => {
	...
	if (isStateOnly) {
		...
		// Check for Virginia
		if (normalizedState === 'virginia' || normalizedState === 'va') {
			return {
				icon: <VirginiaIcon />,
				backgroundColor: VIRGINIA_BACKGROUND,
			};
		}

		// Check for Washington
		if (normalizedState === 'washington' || normalizedState === 'wa') {
			return {
				icon: <WashingtonIcon />,
		...
		// Check for Wisconsin
		if (normalizedState === 'wisconsin' || normalizedState === 'wi') {
			return {
				icon: <WisconsinIcon />,
