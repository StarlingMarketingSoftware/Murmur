...
interface ScrollableTextareaProps extends React.TextareaHTMLAttributes<HTMLTextAreaElement> {
...
const ScrollableTextarea: FC<ScrollableTextareaProps> = ({
	...
	const updateScrollbar = useCallback(() => {
		...
		const newThumbTop = maxThumbTop > 0 ? (scrollTop / maxScrollTop) * maxThumbTop : 0;
...
function getDraftRowHoverRegion(
...
export const DraftedEmails = forwardRef<DraftedEmailsHandle, DraftedEmailsProps>(
	(props, ref) => {
		const {
			draftEmails,
			isPendingEmails,
		...
		useEffect(() => {
			...
			selectedDraft,
			setSelectedDraft,
			props.disableOutsideClickClose,
			lockDraftReviewOpen,
		]);
		...
		const resolvedUsedContactCampaigns = useMemo(() => {
			...
			const other = currentCampaignId
				? all.filter((c) => c.id !== currentCampaignId)
				: all;
		...
		const handleDraftListKeyboardNavigation = useCallback(
			(e: KeyboardEvent) => {
				...
				if (e.key === 'ArrowUp') {
					newIndex =
						hoveredDraftIndex > 0 ? hoveredDraftIndex - 1 : filteredDrafts.length - 1;
				} else {
					newIndex =
						hoveredDraftIndex < filteredDrafts.length - 1 ? hoveredDraftIndex + 1 : 0;
				}
						...
						>
							...
							>
								{!lockDraftReviewOpen && (
									...
									>
										<div
											style={{ width: '16px', height: '2px', backgroundColor: '#000000' }}
										/>
									</Button>
								)}
								<div
								...
								{isRegenSettingsPreviewOpen && !isMobile && !showBottomCounter && (
									<>
										...
										>
											...
											<span className="mx-[10px] whitespace-nowrap flex-shrink-0">
												{'>'}
											</span>
											<button
												type="button"
									...
									<>
										...
										>
											...
											>
												...
												/<a\s+[^>]*href=/i.test(selectedDraft.message) ? (
													<div
														data-hover-description="Revise your draft here. Type out your revisions. Send or delete drafts"
														className="w-full h-full"
													>
				...
				>
					<>
						...
						>
								...
								return (
									...
									>
										...
										<div
											...
											style={
												isMobile ? { width: mobileEmailRowWidth } : { width: '489px' }
											}
											...
											onMouseMove={(e) => {
												...
												setHoveredDraftRow((prev) => {
													if (prev?.draftId === draft.id && prev.region === region)
														return prev;
											...
											onMouseLeave={() => {
												setHoveredDraftRow((prev) =>
													prev?.draftId === draft.id ? null : prev
												);
											}}
											onClick={(e) => {
												const region = getDraftRowHoverRegion(e);
										...
										>
											...
											{hoveredRegion === 'left' && !shouldHighlightRowForUsedContact && (
												<span className="absolute left-0 top-0 w-[156px] h-[5px] bg-[#CCECD4] pointer-events-none rounded-tl-[6px]" />
											)}
											{/* Middle-hover accent bar (from x=156px to right edge, 5px tall) */}
											{hoveredRegion === 'middle' &&
												!shouldHighlightRowForUsedContact && (
													<span className="absolute left-[156px] right-0 top-0 h-[5px] bg-[#FFDE97] pointer-events-none rounded-tr-[6px]" />
												)}
											{/* Rejected indicator */}
											{isRejected && (
												<span
											...
											>
												...
												<button
													...
													onMouseMove={(e) => {
														...
														e.stopPropagation();
														setHoveredDraftRow((prev) => {
															if (prev?.draftId === draft.id && prev.region === 'middle')
																return prev;
											...
											>
												...
												{(() => {
													const topRowMargin = contactTitle ? 'mr-[220px]' : 'mr-[120px]';
													return (
														<>
							...
							{Array.from({
								length: Math.max(0, (isMobile ? 4 : 6) - filteredDrafts.length),
							}).map((_, idx) => (
								<>
					...
					<div className="w-[499px] flex flex-col gap-2 mt-2">
						...
						<div className="relative w-[475px] h-[40px] mx-auto">
							...
							{(() => {
								...
								return props.isSendingDisabled ? (
