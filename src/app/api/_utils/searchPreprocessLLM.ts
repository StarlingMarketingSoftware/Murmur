...
export async function applyHybridLocationOverrides(
	...
	const quickPatterns = [
		{
			pattern: /\bnyc\b/i,
			result: { city: null, state: 'New York', forceCityAny: ['New York', 'Brooklyn'] },
		},
		{
			pattern: /\bphilly\b/i,
			result: { city: 'Philadelphia', state: 'Pennsylvania', forceCityExact: true },
		},
		{
			pattern: /\bdc\b|\bd\.c\./i,
			result: { city: 'Washington', state: 'District of Columbia', forceCityExact: true },
		},
		{
			pattern: /\bla\b(?!.*louisiana)/i,
			result: { city: 'Los Angeles', state: 'California', forceCityExact: true },
		},
	];
