...
interface HistoryLedgerPanelProps {
	historyActions: HistoryAction[];
	onClose: () => void;
}

/**
 * The "History" ledger panel: a backdrop (close-on-outside-click) plus the card
 * listing batched contacts/drafts/sent/received actions with filter tabs.
 *
 * Presentational only — the caller owns the positioning wrapper (BottomPanelsContainer
 * anchors it above the bottom boxes; CampaignCornerPill anchors it above the corner pill).
 * The card carries its own internal z (card z-50 above backdrop z-40); the caller's
 * wrapper governs stacking against surrounding chrome.
 */
...
export default HistoryLedgerPanel;
